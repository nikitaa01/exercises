...
export const GET: APIRoute = async ({ params, cookies }) => {
  ...
  const lastRepetitionsByExercise = await supabase(cookies)
    .from("workouts_repetitions")
    .select("*")
    .eq("exercise_id", exercise_id)
    .order("created_at", { ascending: false })

  const averageRepetitionsByExercise = await supabase(cookies).rpc("get_avg_weight_reps_by_order", { exercise_id_param: exercise_id })
